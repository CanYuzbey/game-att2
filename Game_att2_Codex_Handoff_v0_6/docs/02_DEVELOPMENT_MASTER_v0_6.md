...
consequential NPC needs an independent Goal, something it needs/wants/refuses, and a
world-facing result it is pursuing. It may attempt to break player Will and take a
pre-disclosed useful asset when that claim advances its purpose. The claim may not be
generated by scanning the player's body or wallet at defeat.

The paper-only `DWF-0.1` research package now provides one falsifiable comparison for
the still-open defense and Will values: ordinary Full/Strained Arms Block/Parry,
Full/Strained Legs Evade; Block loss `ceil(D * 0.75 * GuardFactor)`; a minimum `900 ms`
cue; Block locked by `250 ms` before contact; Parry `±90 ms`; Evade `±180 ms`;
independent timing/speed/automation accessibility controls; and bilateral `90 Will`
with `24/30/36` loss after Parrying a Routine/Committed/Critical attack. Ordinary
damage produces zero Will loss, and no body/Blood/Will state shrinks the timing window.

The `G1` Guard comparison begins the captive at `70 Blood` with a complete Full body.
The Guard's own guarding arm is visibly failing before a duty inspection. It accepts a
compatible Full Right Arm or exactly `20 Blood` for replacement/treatment. Blood
payment releases at `50 Blood` with the body intact. Controlled arm payment releases
at `60 Blood` after the provisional `10 Blood` Clean-Stump consequence, with Right Arm
Missing and the stump Controlled for two wound ticks; Left-Arm Punch/Block/Parry,
Kick/Evade, and Headbutt remain.

All of those numbers and G1's identity are `WORKING HYPOTHESIS`, not promoted paper
rules. The source record, formulas, assumptions, toy pacing model, playability
invariant, and rejection criteria are in
`../research/defense_will_npc_balance_v0_1.md`. Owner review must explicitly decide
whether Broken player Will enforces the disclosed claim or allows one final lethal
Defy. No simulator, configuration, test, dependency, engine project, or executable
behavior changed through this amendment.

## 2026-08-22 defense cue, body Block, and Will amendment

For the active Underground City demo, outgoing attack cards resolve without an
attack-side QTE. Short timing input belongs to defense against incoming attacks. A
Yellow threat cue permits Block or Parry. A Red threat cue requires Evade and cannot
be Blocked or Parried. Color must be reinforced by a distinct icon/shape, animation,
or audio language; exact presentation and accessibility assistance remain open.

Block selects another legal usable body part and interposes it before the opponent's
declared target. The original target avoids the direct hit, while the chosen guarding
part becomes the structural recipient, loses Integrity, and may lose dependent
capability. Exact eligible parts, loss, wound/Blood escalation, and input remain open.

Parry is the deliberately difficult precision response. Success prevents the incoming
Integrity, wound, and Blood consequence and reduces the attacker's visible
`Will (İrade)`; a miss applies the original attack without an additional hidden
penalty. Evade is the required answer to Red; success prevents the consequence and
does not create range or reposition state. Exact sources, windows, assistance, and
values remain open.

Broken Will is the active-demo duel's explicit living-surrender trigger. It opens the
limb bargain only while the opponent remains alive, the agreed limb is transferable,
and the player remains a credible threat. It does not itself damage or transfer the
limb. Starting Will, Parry loss, recovery, other mutation sources, and actor exceptions
remain open.

This amendment is active-demo paper authority only. It does not alter Combat Rules
...
costs, combo sequencing, and any bounded conversion relationship with Blood remain
open. Mana capacity increases as combat rounds pass so early turns expose a smaller
decision surface and later turns permit larger card sequences. The starting amount,
per-round increase, cap, full/partial refresh, and carryover remain open.

The owner also selected the demo's ordinary encounter-reward split. Killing the
opponent grants a positive Blood reward sourced from that dead opponent and
permanently removes access to that opponent's limb reward for the current day.
Accepting a living, state-derived surrender grants access to the agreed limb through
the Grafting Table but grants no kill-Blood reward. The exact Blood yield, collection
timing and presentation, interaction with wound state, and post-reward cap remain
open. Blood is not awarded merely for dealing damage or reaching surrender.

The reward economy must produce learnable same-day route pressure rather than hidden
adaptive loot. A poorly played attempt may leave the player with Blood but no access
to the desired limb, or with the desired limb available but insufficient Blood for
the intended downstream use. On death, the same day restores the same underlying
opportunities; protagonist knowledge, not wallet-sensitive limb spawning, should let
the player choose a better sequence on the next attempt. Exact opportunity placement,
graft Blood requirement, and success threshold remain open.

The active-demo duel must not admit a repeatable no-change loop. The current working
hypothesis combines growing Mana with irreversible or finite combat pressure: a full
round advances the rising Mana clock until its cap; at or beyond that cap, a full
round must consume or worsen a non-renewable encounter fact such as Blood, integrity,
wound severity, finite item/charge, or capability. A temporary status/commitment that
later expires is insufficient by itself. Defense and restoration may not return all
meaningful state for free. Exact anti-stall mechanics and numbers remain open.
...
resource is named `Mana`; its values, refresh, order, exact condition scale, and
thresholds remain open. Earlier paper-only one-Main action economy and
Clinch/Engaged/Distant range direction are superseded for the active demo. No range
or reposition state belongs in the active demo. Frozen simulator runtime/config/tests
...
Killing the actor removes access to that limb reward for the current day and instead
grants the active-demo kill-Blood reward described above.
