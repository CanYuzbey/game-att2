...
documents 27 through 37. In particular, document 29 remains the paper authority for
Attention Slots and the current tactical-opportunity lifecycle. The full deckbuilder,
card acquisition, active-deck construction, draw/discard rules, and death persistence
discussed here remain working design exploration.
...
5. which part of the deck was authored by the player rather than automatically
   assigned.
...
The next unresolved macro topic is **card access and deck construction**, beginning
with the source of technique discovery and the role—if any—of death in preserving
knowledge. Resolve that architecture before exact card counts, reward frequencies,
or draw/discard mathematics.
